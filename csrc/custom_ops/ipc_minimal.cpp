...
static inline void enable_peer_access_all_visible(int my_dev) {
  int ndev = 0;
  C10_CUDA_CHECK(cudaGetDeviceCount(&ndev));
  for (int d = 0; d < ndev; ++d) {
    if (d == my_dev) continue;
    int can = 0;
    C10_CUDA_CHECK(cudaDeviceCanAccessPeer(&can, my_dev, d));
    if (can) {
    ...
    c10::intrusive_ptr<c10d::ProcessGroup> pg, int64_t capacity_elems)
{
  ...
  // Local allocations
  C10_CUDA_CHECK(cudaMalloc(&ws.data_triple, 3 * ws.comm_size_bytes));
  C10_CUDA_CHECK(cudaMalloc(&ws.ctrl_dev,    8 * sizeof(int)));
  int ctrl_host[8] = {};
  ...
  C10_CUDA_CHECK(cudaMemcpy(ws.ctrl_dev, ctrl_host, sizeof(ctrl_host), cudaMemcpyHostToDevice));
  ...
  // Exchange CUDA IPC handles via NCCL PG (NCCL requires CUDA tensors!)
  cudaIpcMemHandle_t local_handle{};
  C10_CUDA_CHECK(cudaIpcGetMemHandle(&local_handle, ws.data_triple));

  auto dev_b = torch::TensorOptions().dtype(torch::kByte).device(torch::kCUDA, dev);

  at::Tensor send_cuda = torch::empty((long)sizeof(cudaIpcMemHandle_t), dev_b);
  C10_CUDA_CHECK(cudaMemcpy(send_cuda.data_ptr(), &local_handle,
                            sizeof(cudaIpcMemHandle_t), cudaMemcpyHostToDevice));

  std::vector<at::Tensor> recvs(ws.world_size);
  for (int i = 0; i < ws.world_size; ++i) {
    recvs[i] = torch::empty((long)sizeof(cudaIpcMemHandle_t), dev_b);
  }

  // ProcessGroup::allgather expects: vector<vector<Tensor>>& outputs, vector<Tensor>& input
  std::vector<std::vector<at::Tensor>> outputs(1);
  outputs[0] = recvs;
  std::vector<at::Tensor> inputs{send_cuda};

  c10d::AllgatherOptions ag;
  pg->allgather(outputs, inputs, ag)->wait();

  // Open remote buffers (copy handles back to host)
  ws.opened_ptrs.resize(ws.world_size, nullptr);
  ws.is_self.resize(ws.world_size, 0);
  for (int r = 0; r < ws.world_size; ++r) {
    cudaIpcMemHandle_t h{};
    C10_CUDA_CHECK(cudaMemcpy(&h, outputs[0][r].data_ptr(),
                              sizeof(cudaIpcMemHandle_t), cudaMemcpyDeviceToHost));
    if (r == ws.rank) {
