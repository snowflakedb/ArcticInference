...
struct IpcTripleWorkspace {
  ...
  std::vector<uint8_t> is_self;   // same length as opened_ptrs; 1 if self
...
// Build triple buffers + ctrl block, exchange IPC handles via ProcessGroup,
...
// Prefill helper from CUDA file
void lamport_fill_neg_zero(void* ptr, size_t nbytes, cudaStream_t stream);
