...
def _schedule_configs() -> List[List[str]]:
    sorted_configs = sorted(VLLM_CONFIGS.items(),
...
class BatchServerManager:
    ...
    def start_batch(self, batch_idx: int, batch_configs: List[str]):
        ...
        for i, config_name in enumerate(batch_configs):
            port = BASE_PORT + i
            gpus_needed = VLLM_CONFIGS[config_name].get("tensor_parallel_size", 1)
            ...
            print(f"  -> Launching '{config_name}' on port {port} with GPUs {gpu_ids}...")
    ...
    def teardown_current_batch(self):
        ...
        for name, p in self.processes.items():
            if p.poll() is None:
                print(f"  -> Terminating '{name}' on port {self.port_map.get(name)}")
                p.terminate()
                try:
                    p.wait(timeout=10)
    ...
    def _wait_for_server_ready(process: subprocess.Popen, port: int):
        ...
        while True:
            if process.poll() is not None:
                raise RuntimeError(f"Server on port {port} terminated unexpectedly. "
...
def pytest_generate_tests(metafunc):
    ...
    """
    # Parallel (batch) tests for accuracy
        ...
        if metafunc.function.__name__ == "test_batch_accuracy":
            for batch_idx, configs in enumerate(batches):
                for task_name, task_obj in ACCURACY_TASKS.items():
                    all_specs.append({
                        "batch_idx": batch_idx, "configs": configs,
                        "task_name": task_name, "task_obj": task_obj,
                    })
                    all_ids.append(f"b{batch_idx}-{task_name}")
        metafunc.parametrize("batch_spec", all_specs, ids=all_ids, indirect=True)

    # Serial (per-config) tests for performance and json_mode
    if "benchmark_spec" in metafunc.fixturenames:
        batches = metafunc.config.vllm_batches
        all_specs, all_ids = [], []
        task_map = { "test_performance": PERFORMANCE_TASKS, "test_json_mode": JSON_MODE_TASKS }
            ...
            for batch_idx, configs in enumerate(batches):
                for config_name in configs:
                    for task_name, task_obj in task_map[test_func_name].items():
                        all_specs.append({
                            "batch_idx": batch_idx, "config_name": config_name,
                            "task_name": task_name, "task_obj": task_obj,
                        })
                        all_ids.append(f"b{batch_idx}-{config_name}-{task_name}")
        metafunc.parametrize("benchmark_spec", all_specs, ids=all_ids, indirect=True)
